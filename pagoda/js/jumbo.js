...
$(document).ready(function() {
	...
	popComm = {
		...
		call : function(id, state){
			...
			if(state){
				...
				posY = ($(window).scrollTop() + ($(window).outerHeight() / 2)) + 70;
	...
	$('.sec_event .event_box').hover(function lightHover(){
		$(this).addClass('on');
		$(this).parents().find('.light').addClass('on');
		$(this).parents().find('.light_off').removeClass('on');
	}, function() {
		$(this).removeClass('on');
		$(this).parents().find('.light').removeClass('on');
		$(this).parents().find('.light_off').addClass('on');
	});
