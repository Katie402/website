/* ***************************************************************************
	JkBbs 클래스 기반의 게시판 페이지에서 사용하는 공통 js 파일

	r_code 는 jk_bbs_conf 테이블에 등록된 게시판 식별코드이며,
	목록 파일은 {$r_code}.php
	보기 파일은 {$r_code}_view.php
	쓰기 파일은 {$r_code}_write.php
	형식으로 구성해야 합니다.
*************************************************************************** */
...
function go_regist(cmd){
	...
	if(use_editor == "Y"){
		$(".editor").each(function(){
			...
			if($("#frm_form [name='"+editor+"']").val() == "<p>&nbsp;</p>")
				$("#frm_form [name='"+editor+"']").val("");
		});
	}
